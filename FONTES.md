...
2. **Instabilidade**: instáveis — DataSUS FTP, PDET/RAIS, INEP, dumps da Receita; estáveis — IBGE, BCB, TSE, SICONFI, CVM, Câmara. O INEP apagou séries em 2022 e nunca republicou completas; a série do SNIS saiu do ar. **A camada `raw/` imutável é a garantia de reprodutibilidade. 2026 é ano eleitoral: espelhar o que importa antes de dezembro.**
   ...
   **Estado da onda 1 (verificado em 27/07/2026)**:
   - **INEP — espelhado.** 190 MB no R2: censo escolar 2020–2024 e IDEB anos iniciais/finais 2023. Duas descobertas: o servidor manda cadeia TLS incompleta (resolvido com o intermediário versionado em `config/ca/inep.pem`), e **o IDEB de 2019 e 2021 já responde 404** — parte da série se perdeu antes de começarmos.
   - **SNIS — não espelhável hoje, por nenhum caminho programático.** O sistema encerrou a coleta em 2023 (o SINISA o sucede desde 2024) e a série histórica ficou inacessível: `app4.mdr.gov.br` não resolve DNS, `app4.cidades.gov.br` e `dadosabertos.cidades.gov.br` respondem `403 Access Denied` de appliance (não é User-Agent — testado). **O catálogo federal foi consultado com chave de API e fechou a questão**: o conjunto "SNIS - Série Histórica" existe (dados de 25/04/2023), mas tem **um único recurso, em HTML, apontando para o `app4.mdr.gov.br` morto** — o `dados.gov.br` não hospeda arquivo nenhum do SNIS, apenas registra um link para o vazio. O SINISA não tem conjunto no catálogo. É o pior caso do risco descrito acima: o índice oficial sobrevive ao dado que ele indexa. Resta download manual pelo navegador ou fonte de terceiros (Base dos Dados).
